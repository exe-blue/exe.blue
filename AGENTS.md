...
- **Assignee = 호출 (기본).** 너에게 이슈가 할당되면, 너는 호출된 것이다
- **Delegate = 호출 (IDE agent 예외).** Cursor·Codex·Sentry Seer 같은 IDE/플랫폼 통합형 agent는 assignee가 ORIGIN(또는 인간)이고 별도의 `delegate` 필드로 호출된다. 본 예외의 자세한 룰은 ADR-045 §2.3 + ADR-046 §3.3
...
| 상황 | 매개층 |
|---|---|
| 새 작업 할당 (인간/순수 AI) | Linear (assignee 지정) |
| 새 작업 할당 (IDE agent) | Linear (delegate 지정, assignee는 ORIGIN 유지) |
...
- 작업 단위는 작업자 유형에 종속된다 — AI agent는 원자단위(ADR-046), 인간은 티셔츠 단위(ADR-046 §3.2)
- 자기 브랜치에서 작업한다 (명명: `<agent>/<linear-id>-<slug>` 예: `claude/REB-236-regx-spec`, `cursor/profile-page-functionality-447e`)
- PR을 올린다. 본문에 Linear 이슈 명시 (`Closes REB-236`). PR title은 부여 코드 head 원칙(ADR-046 §3.5)
...
- 인간 assignee의 이슈를 atom으로 분해하지 않는다 (ADR-046 §3.3)
...
| Agent | Platform | Role | 호출 방식 | 작업 단위 |
|---|---|---|---|---|
| VEILE | Claude.ai 웹 | 설계·리뷰 | Linear assignee + Slack DM | 채팅 세션 |
| FORGE | Claude Code | 코드 실행 | Linear branch + 자동 동기화 | 원자 |
| SEER | GPT | 보조 분석 | Linear comment 멘션 | 채팅 |
| LUON | Gemini | 보조 분석 | Linear comment 멘션 (Slack 미참여) | 채팅 |
| Cursor | IDE Background Agent | UI/React/component | Linear delegate (assignee=ORIGIN, delegate=Cursor) | 원자 |
| Codex | OpenAI ChatGPT Plus/Pro | 백엔드/Edge/refactor | Linear delegate | 원자 |
| Sentry Seer | Sentry Auto-fix | 프로덕션 회귀 root cause | Sentry → Linear 자동 | 원자 |
| GitHub agent | GitHub | repo 이벤트 동기화 | webhook → Linear | (작업자 아님) |
| 인간 백엔드 협업자 | — | KAIROS pipeline, Edge Function | Linear assignee | 티셔츠 (S/M/L/XL) |

새 AI 에이전트가 합류하면 이 표에 추가. 인간 작업자도 동일 표에 row로 등재 — 다만 본 문서의 자동 라우팅 룰은 AI 한정 (ADR-046 §3.3).
...
- Linear AiX 팀 9개 신규 라벨: `agent:cursor`, `agent:codex`, `agent:seer`, `ui`, `backend`, `edge-function`, `migration`, `refactor`, `needs:respec`
- Cursor: AIX-243 reference 운영 (Done 2026-05-11), AIX-264 자동 라우팅 검증 진행 중 (PR #546)
- Codex: 2026-05-14 활성화, Custom prompt 적용, AIX-269 라인부터 검증 예정
- Sentry Seer: 2026-05-14 활성화
- Triage Rules: 2026-05-14 활성화 (Team Settings > Triage > Rules)

### ADR-046 (원자단위 위임)

- 한 PR = 한 의도 (AI 라인 한정)
- 분해 매트릭스 (§3): AI agent → 원자, 인간 → 티셔츠(S/M/L/XL), ORIGIN 수동 → 원자(estimate 없음)
- 분해 자문 3 step (AI 한정): 동사 수, 독립 검증 가능성, 독립 rollback 가능성
- Hub issue 패턴: AIX-80 (hub) → AIX-264/265/266 (atomic), AIX-206 (hub) → AIX-267/268/269/270 (atomic)
- 명명 룰 (§3.5): 부여 코드를 head로 (`[AIX-264] ... (AIX-80 sub-A)`)
- 라우팅 룰 (§3.3): label `agent:*` AND assignee is agent
